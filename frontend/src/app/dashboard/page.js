...
function DashboardPage() {
    ...
    const [clientName, setClientName] = (0, react_1.useState)("Client");
    const [ibanInfo, setIbanInfo] = (0, react_1.useState)(null);
    const [showAccountModal, setShowAccountModal] = (0, react_1.useState)(false);
    const [showTransferModal, setShowTransferModal] = (0, react_1.useState)(false);
    const [showCreateAccountModal, setShowCreateAccountModal] = (0, react_1.useState)(false);
    const { show } = (0, ToastContext_1.useToast)();
    const [modalBanner, setModalBanner] = (0, react_1.useState)("");
    const [accounts, setAccounts] = (0, react_1.useState)([]);
    const [activeAccountId, setActiveAccountId] = (0, react_1.useState)(null);
    const [displayedCardIndex, setDisplayedCardIndex] = (0, react_1.useState)(0);
    const [activityHistory, setActivityHistory] = (0, react_1.useState)([]);
    const [newAccountName, setNewAccountName] = (0, react_1.useState)("");
    const [beneficiaries, setBeneficiaries] = (0, react_1.useState)([]);
    const [showBeneficiaryModal, setShowBeneficiaryModal] = (0, react_1.useState)(false);
    // Load account data (transactions and activity history) for a specific account
    const loadAccountData = (0, react_1.useCallback)(async (accountId) => {
        if (!accountId)
            return;
        try {
            // Load transactions
            const txns = await api_1.default.listTransactions([accountId]);
    ...
    const switchAccount = (0, react_1.useCallback)(async (accountId) => {
        ...
        async function load() {
            try {
                ...
                const [profile, list] = await Promise.all([
                    api_1.default.getClient(clientId),
                    api_1.default.listAccounts(clientId),
                ]);
    ...
    (0, react_1.useEffect)(() => {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                const list = await api_1.default.listAccounts(clientId);
                setAccounts(list);
                ...
                if (currentAccount) {
                    ...
                    const txns = await api_1.default.listTransactions([activeAccountId]);
                    const formatted = txns.map(t => ({
                        id: t.id,
                        date: t.createdAt,
                        label: t.label,
    ...
    const income = (0, react_1.useMemo)(() => transactions.filter(t => t.amount > 0).reduce((s, t) => s + t.amount, 0), [transactions]);
    const expense = (0, react_1.useMemo)(() => transactions.filter(t => t.amount < 0).reduce((s, t) => s + Math.abs(t.amount), 0), [transactions]);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 card">
          ...
          <div className="divide-y divide-white/10 max-h-64 overflow-y-auto">
            {activityHistory.length === 0 ? (<p className="text-muted text-sm py-4">Pas encore d'historique d'activités</p>) : (activityHistory.map((activity) => (<div key={activity.id} className="py-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`h-9 w-9 rounded-full border flex items-center justify-center ${activity.type === "create"
        ...
        <div className="card">
          ...
          <div className="flex items-center gap-3">
            <button className={"icon-btn h-8 w-8 " + (accounts.length === 0 || displayedCardIndex <= 0 ? "opacity-40 pointer-events-none" : "")} aria-label="Précédent" onClick={() => {
            ...
            <div className="relative flex-1">
              {(() => {
            if (!displayedAccount)
                return <div className="text-sm text-muted">Aucun compte</div>;
            const isActive = displayedAccount.id === activeAccountId;
            return (<div className={"p-1 rounded-2xl " + cardBgByIndex(displayedCardIndex)}>
                    <div className="relative">
                      <button className="absolute -top-2 -right-2 h-7 w-7 rounded-full bg-red-500/15 border border-red-500/30 text-red-400 hover:bg-red-500/25 z-10 flex items-center justify-center" aria-label="Supprimer le compte" onClick={async (e) => {
                    e.stopPropagation();
                    if (!confirm("Supprimer ce compte ?"))
                        return;
                    const accountName = displayedAccount.name || "Compte";
                    await api_1.default.deleteAccount(displayedAccount.id);
                    ...
                    show("Compte supprimé", "success");
                      ...
                      <button className="w-full text-left" onClick={() => switchAccount(displayedAccount.id)}>
                        <BankCard balance={displayedAccount.balance} holder={clientName} last4={isActive ? displayedAccount.iban.slice(-4) : "****"}/>
                      </button>
                    </div>
                  </div>);
        })()}
            </div>
            <button className={"icon-btn h-8 w-8 " + (accounts.length === 0 || displayedCardIndex >= accounts.length - 1 ? "opacity-40 pointer-events-none" : "")} aria-label="Suivant" onClick={() => {
            if (accounts.length === 0)
                return;
          ...
          <div className="mt-2 text-center text-xs text-muted">
            {accounts.length > 0 ? (<span>{displayedCardIndex + 1} / {accounts.length}</span>) : (<span>0 / 0</span>)}
          </div>
      ...
      <div className="card">
        ...
        <div className="divide-y divide-white/10">
          {loading ? (<p className="text-muted text-sm py-4">Chargement…</p>) : transactions.length === 0 ? (<p className="text-muted text-sm py-4">Pas encore de transactions</p>) : (transactions.map((t) => (<div key={t.id} className="py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
    ...
    {/* Account Info Modal */}
    {showAccountModal && ibanInfo && (<Modal onClose={() => setShowAccountModal(false)}>
        ...
        <div className="space-y-2 text-sm">
          ...
          {ibanInfo.createdAt && (<div className="flex items-center justify-between"><span className="text-muted">Date de création</span><span className="font-medium">{new Date(ibanInfo.createdAt).toLocaleDateString("fr-FR")}</span></div>)}
    ...
    {/* Beneficiary Modal */}
    {showBeneficiaryModal && activeAccountId && (<BeneficiaryModal accountId={activeAccountId} beneficiaries={beneficiaries} onClose={() => setShowBeneficiaryModal(false)} onUpdate={(updated) => {
                setBeneficiaries(updated);
                localStorage.setItem(`beneficiaries_${activeAccountId}`, JSON.stringify(updated));
            }}/>)}

    {/* Transfer Modal */}
    {showTransferModal && (<TransferModal fromIban={ibanInfo?.iban || ""} balance={balance} beneficiaries={beneficiaries} onClose={() => setShowTransferModal(false)} onSuccess={async (amount, toIban) => {
                try {
                    await api_1.default.transfer(ibanInfo?.iban || "", toIban, amount);
    ...
    {showCreateAccountModal && (<Modal onClose={() => setShowCreateAccountModal(false)}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" className="input-minimal w-full" placeholder="Compte courant" value={newAccountName} onChange={(e) => setNewAccountName(e.target.value)}/>
          </div>
          <div className="flex gap-3 mt-6">
            ...
            <button className="btn-primary flex-1" onClick={async () => {
                try {
                    ...
                    if (!clientId)
                        return;
                    const created = await api_1.default.createAccount(clientId, newAccountName || undefined);
...
function BankCard({ balance, holder, last4 }) {
    return (<div className="relative overflow-hidden rounded-2xl border border-white/10 p-5 bg-gradient-to-br from-white/5 to-white/0">
...
function TransferModal({ fromIban, balance, beneficiaries, onClose, onSuccess }) {
    const [toIban, setToIban] = (0, react_1.useState)("");
    const [amount, setAmount] = (0, react_1.useState)("");
    const [selectedBeneficiaryId, setSelectedBeneficiaryId] = (0, react_1.useState)("");
    ...
    return (<Modal onClose={onClose}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {beneficiaries.length > 0 && (<div>
            <label className="block text-sm font-medium mb-2">Choisir un bénéficiaire</label>
            <select className="input-minimal w-full" value={selectedBeneficiaryId} onChange={(e) => setSelectedBeneficiaryId(e.target.value)}>
              <option value="">Sélectionner un bénéficiaire ou saisir manuellement</option>
              {beneficiaries.map(b => (<option key={b.id} value={b.id}>
        ...
        <div>
          ...
          <input type="text" className="input-minimal w-full" placeholder="FR76 XXXX XXXX XXXX XXXX XXXX XXX" value={toIban} onChange={(e) => {
        ...
        <div>
          ...
          <input type="number" step="0.01" min="0.01" className="input-minimal w-full" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} required/>
...
function BeneficiaryModal({ accountId, beneficiaries, onClose, onUpdate }) {
    const [iban, setIban] = (0, react_1.useState)("");
    ...
    const handleVerify = async () => {
        ...
        setLoading(true);
        try {
            const result = await api_1.default.verifyBeneficiary(iban.trim(), firstName.trim(), lastName.trim());
    ...
    const addBeneficiary = (actualFirstName, actualLastName, isInBank) => {
    ...
    return (<Modal onClose={onClose}>
      ...
      <div className="space-y-4 mb-6 pb-6 border-b border-white/10">
        ...
        <div>
          ...
          <input type="text" className="input-minimal w-full" placeholder="FR76 XXXX XXXX XXXX XXXX XXXX XXX" value={iban} onChange={(e) => setIban(e.target.value)}/>
        </div>
        <div>
          <label className="block text-sm font-medium mb-2">Prénom</label>
          <input type="text" className="input-minimal w-full" placeholder="Prénom" value={firstName} onChange={(e) => setFirstName(e.target.value)}/>
        </div>
        <div>
          <label className="block text-sm font-medium mb-2">Nom</label>
          <input type="text" className="input-minimal w-full" placeholder="Nom" value={lastName} onChange={(e) => setLastName(e.target.value)}/>
        </div>
        {verificationResult && !verificationResult.exists && (<div className="p-3 bg-yellow-500/10 border border-yellow-500/30 rounded text-sm text-yellow-400">
            Ce bénéficiaire ne fait pas partie de notre banque.
          </div>)}
        {verificationResult && verificationResult.exists && !verificationResult.verified && (<div className="p-3 bg-orange-500/10 border border-orange-500/30 rounded text-sm text-orange-400">
      ...
      {/* Confirmation dialog */}
      {showConfirmDialog && verificationResult && (<div className="fixed inset-0 z-[70] flex items-center justify-center">
          ...
          <div className="relative glass border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-glow">
            ...
            {!verificationResult.exists ? (<p className="text-sm mb-4">
      ...
      <div>
        ...
        {beneficiaries.length === 0 ? (<p className="text-muted text-sm py-4">Aucun bénéficiaire enregistré</p>) : (<div className="space-y-2 max-h-64 overflow-y-auto">
            {beneficiaries.map(b => (<div key={b.id} className="flex items-center justify-between p-3 bg-white/5 rounded border border-white/10">
                <div>
                ...
                <button className="icon-btn text-red-400" onClick={() => handleDelete(b.id)} aria-label="Supprimer">
                  ✕
                </button>
              </div>))}
          </div>)}
